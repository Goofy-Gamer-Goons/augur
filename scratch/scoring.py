...
def calculate_watchers_score(repo_id, repo_info):
    score = 0
    count = repo_info['watchers']
...
def calculate_forks_score(repo_id, repo_info):
    score = 0
    count = repo_info['forks']
...
def calculate_stars_score(repo_id, repo_info):
    score = 0
    count = repo_info['stars']
    if count >= 1 and count <= 5:
        score = 1
    elif count >= 6 and count <= 15:
        score = 2
...
def calculate_code_of_conduct_score(repo_id, repo_info):
    score = 0
    location = repo_info['code_of_conduct_file']
    if location != None:
        score = 5
    return score

def calculate_license_file_score(repo_id, repo_info):
    score = 0
    location = repo_info['license_file']
    if location != None:
        score = 5
    return score

def calculate_good_first_issue(repo_id, repo_info):
    score = 0
    repo_info = repo_info
...
def calculate_last_commit_score(repo_id, repo_info):
    score = 0
    now = datetime.now()
    repo_last_updated_raw = repo_info['last_updated']
...
def calculate_best_practices_score(repo_id, repo_info):
    score = 0
    repo_info = repo_info
...
def calculate_last_release_score(repo_id):
    ...
    release_info = requests.get(f"{server_location}/{repo_id}/toss-release-info").json()
    if len(release_info) == 0:
        return score

    last_published_date_raw = release_info[0]['release_published_at']

    last_published_date = datetime.strptime(last_published_date_raw, "%Y-%m-%dT%H:%M:%S.%fZ")
    months = relativedelta(now, last_published_date).months
...
def calculate_repo_score(repo_id):

    print(repo_id)
    print("--------")

    repo_info = requests.get(f"{server_location}/{repo_id}/toss-repo-info").json()[0]
    watchers_score = calculate_watchers_score(repo_id, repo_info)
    forks_score = calculate_forks_score(repo_id, repo_info)
    stars_score = calculate_stars_score(repo_id, repo_info)
    code_of_conduct_score = calculate_code_of_conduct_score(repo_id, repo_info)
    license_file_score = calculate_license_file_score(repo_id, repo_info)
    good_first_issue = calculate_good_first_issue(repo_id, repo_info)
    last_commit_score = calculate_last_commit_score(repo_id, repo_info)
    best_practices_score = calculate_best_practices_score(repo_id, repo_info)
    last_release_score = calculate_last_release_score(repo_id)
    total_score = watchers_score + forks_score + stars_score + code_of_conduct_score + license_file_score + good_first_issue + last_commit_score + best_practices_score + last_release_score

    return total_score

    # print("watchers_score:", watchers_score)
    # print("forks_score:", forks_score)
    # print("stars_score:", stars_score)
    # print("code_of_conduct_score:", code_of_conduct_score)
    # print("license_file_score:", license_file_score)
    # print("good_first_issue:", good_first_issue)
    # print("last_commit_score:", last_commit_score)
    # print("best_practices_score:", best_practices_score)
    # print("last_release_score:", last_release_score)
    # print(total_score)

calculate_repo_score(1)